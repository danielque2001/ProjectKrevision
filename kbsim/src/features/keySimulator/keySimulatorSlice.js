  ...
  reducers: {
    ...
    parseKLE: (state, action) => {
      ...
      for (let i in inputLayout) {
        ...
        inputLayout[i] = inputLayout[i].match(
          /(["'])(?:(?=(\\?))\2.)*?\1|([{])(?:(?=(\\?))\2.)*?([}])/g
        );
        if (!inputLayout[i]) {
          state.highlight = { borderColor: '#ff0033' };
      ...
      for (let x = 0; x < inputLayout.length; x++) {
        ...
        for (let y = 0; y < inputLayout[x].length; y++) {
          ...
          if (
            ...
            let keyFormat = inputLayout[x][y]
              .substring(1, inputLayout[x][y].length - 1)
              .replace(/\s/g, '');
                ...
                if (formatTuple.length === 2) {
                  switch (formatTuple[0]) {
                    ...
                    case 'c':
                      // trim quotes
                      if (
                        formatTuple[1].charAt(0) === '"' &&
                        formatTuple[1].charAt(formatTuple[1].length - 1) === '"'
                      ) {
                        formatTuple[1] = formatTuple[1].substring(
                          1,
                          formatTuple[1].length - 1
                        );
                      }
          ...
          // if the current item is a key, parse the legends
            ...
            // trim quotes
            let legends = inputLayout[x][y].substring(
              1,
              inputLayout[x][y].length - 1
            );
      ...
      for (let x in inputLayout) {
        for (let y in inputLayout[x]) {
          ...
          let primaryLegend = parseLegends(
            inputLayout[x][y].legend,
            inputLayout[x][y].sublegend
          );
...
export const { parseKLE, keyDown, keyUp, setKeyboardColor } =
  keySimulatorSlice.actions;
